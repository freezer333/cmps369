...
<p>  When stderr is not a TTY, <code>Date#toUTCString()</code> is used, making it more useful for logging the debug information as shown below:<br>  <em>(NOTE: Debug now uses stderr instead of stdout, so the correct shell command for this example is actually <code>DEBUG=* node example/worker 2&gt; out &amp;</code>)</em></p>
...
<p>  The &quot;<em>&quot; character may be used as a wildcard. Suppose for example your library has debuggers named &quot;connect:bodyParser&quot;, &quot;connect:compress&quot;, &quot;connect:session&quot;, instead of listing all three with <code>DEBUG=connect:bodyParser,connect.compress,connect:session</code>, you may simply do `DEBUG=connect:</em><code>, or to run everything using this module simply use</code>DEBUG=*`.</p>
